...
sid       anchor   c2\_2025   sunnystation   dirtyrat   benign
...
&#x20;   http.user\_agent; content:"NetSupport Manager/1.3"; fast\_pattern;
...
c2\_2025        48 alerts    ->  38.146.28.242:1203
...
track by\_src    one window per source IP, so two infected hosts each get
...
&#x20;   tls.cert\_subject; content:"O=Global Security"; content:"CN=example.com";

&#x20;   tls.cert\_issuer;  content:"O=Global Security"; content:"CN=example.com";
...
alert tcp $HOME\_NET any -> $EXTERNAL\_NET any (

&#x20;   msg:"LOCAL NetSupport RAT - interactive session active, operator at keyboard";

&#x20;   flow:established,to\_server;

&#x20;   content:"NC\_DATA|0a|"; fast\_pattern;

&#x20;   content:"CID="; distance:0;

&#x20;   threshold: type limit, track by\_src, count 1, seconds 300;
...
Buffers such as `http.uri` and `http.user\_agent` only exist after Suricata's
...
POST http://38.146.28.242/fakeurl.htm   NC\_DATA
...
\*\*Result.\*\* 3 alerts on c2\_2025, 0 everywhere else.
...
c2\_2025         48 check-ins, then a human took over     two hands-on sessions
...
\*\*What it cannot do.\*\* `NC\_DATA` is one product's protocol keyword — a hands-on
